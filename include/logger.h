...
namespace mylog {
  class Logger {
    ...
    public: 
      ...
      Logger(int level, std::string filename) : level(level), os(std::ofstream(filename).rdbuf()) {}
      ...
      static std::shared_ptr<Logger> create_logger(int level, std::string filename) {
